...
//!
//! These replace the old in-library runner (src/integration_tests.rs, driven
//! from a C++ main) with ordinary `cargo test` functions.  What a cargo test
//! cannot do itself is bring up a ceph context and a SAL driver -- libtest owns
//! main() -- so that happens in C++ behind the `rgw_test_env_*` API declared
...
//
// These previously ran through our hand-rolled runner in integration_tests.rs.
// As real #[tokio::test] functions they get a genuine Tokio runtime, which is
// what lets stream_get (concurrent multipart uploads) run -- our old
// futures::executor block_on had no reactor.
// ---------------------------------------------------------------------------
...
// Previously skipped: stream_get drives concurrent multipart uploads that
// require a Tokio runtime, which futures::executor could not provide.  A real
// #[tokio::test] runtime makes it runnable.  Note that our put_part bottoms out
// in a blocking FFI call, so the parts serialize rather than truly overlap;
// this exercises the streaming write path, not concurrency.
#[tokio::test]
...
// ---------------------------------------------------------------------------
// Coverage beyond the conformance suite (ported from integration_tests.rs)
// ---------------------------------------------------------------------------
