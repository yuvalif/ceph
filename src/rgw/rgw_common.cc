				  ...
				  const uint64_t op)
{
  ...
  auto identity_policy_res = eval_identity_or_session_policies(s->iam_user_policies, s->env, op, ARN(bucketkey));
  if (identity_policy_res == Effect::Deny) {
    return -EACCES;
  }
  ...
  if (!s->session_policies.empty()) {
    auto session_policy_res = eval_identity_or_session_policies(s->session_policies, s->env, op, ARN(bucketkey));
    if (session_policy_res == Effect::Deny) {
        return -EACCES;
    }
    if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
